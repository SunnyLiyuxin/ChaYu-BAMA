...
**与 §5.4 `/api/chat` 的关系（2026-07-19 更新）**：工作台的自由提问需求已由 §5.4 `/api/chat` 实现——前置一次「意义评判」LLM 拒绝无意义输入（如「？」），有意义输入把原文作为 `directive` 透传到对应生成链路（文案复用 5.1/5.2，物料复用 6.1，本次给 asset 链补了 `directive` 透传）。§5.4 与本入口职责不同：
- §5.4 `/api/chat`：工作台已**前置选定 tea_id + 链路 + 模式**，只做「评判意义 + 透传 directive」，不识别茶品。
- §5.3 NL 入口：从自由文本**识别茶品 + 判链路**（intent LLM），适用于"用户没在 UI 选茶、直接敲一句话"的开放域场景。

故 NL 入口（识别茶品）仍搁置——前端目前仍走 UI 选茶 + §5.4 自由提问，未引入"不选茶直接敲一句话"的开放域输入框。
