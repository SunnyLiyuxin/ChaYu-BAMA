...
> 状态：已实现并合并。
> 范围：把 GLM（学校提供、自定义 base_url）接入 3 个文本生成接口；**不碰真图 / 真视频**（真视频仍是 P2 fallback；真实生图后续已独立接入，现图源为豆包 Seedream，见 `POST /api/image/generate` / docs/接口文档.md §6.2，不在本方案范围内）。
> 历史背景：本方案记录的是「LLM 接入层 3 / 层 4 文本字段」的决策与实施，其中所有"真实生图未接"的描述均为当时的范围边界；真实生图能力在本方案落地之后才新增，已更新到现行文档（README / CLAUDE / 技术架构 / 接口文档）。
