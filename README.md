...
http://localhost:8000/docs      # 直连后端（仅本地联调；生产 CORS 已收紧，跨域默认拦）
```
...
> **CORS 已收紧**（2026-07-19）：生产默认 `allow_origins=[]`（同源 only）。Docker 一体化部署下前端与 `/api` 经 nginx 同 origin，浏览器不发 `Origin` 头、不触发预检，天然不需要 CORS。仅「浏览器直连后端 8000」联调时在 `backend/.env` 配 `CORS_ALLOWED_ORIGINS=http://localhost:8080`。`allow_credentials=false`、`allow_methods` 收到 `GET,POST,OPTIONS`、`allow_headers` 收到 `Content-Type`。详见接口文档 §1.5。
...
当前限制：真实视频生成（video-asset 等）走 fallback、生产鉴权与安全配置尚未接入；CORS 已收紧为同源 only（详见接口文档 §1.5）。其余能力边界见接口文档 §8 / §10。
